...
abstract class Piece {
  ...
  final val pointValue: Int = grid.popCount
  final val offSetToFirstOnPosition: Int = cachedOccupancyGrid(0).zipWithIndex.filter(_._1 == true)(0)._2
...
object Piece {
  ...
  def getElGrid(size: Int, boardSizeInfo: BoardSizeInfo): OccupancyGrid = {
    val grid = getGrid(size, size, filled = true, boardSizeInfo)
